...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const [goodResult, badResult, mclResult, memoryResult] = await Promise.all([
        client.query('SELECT COUNT(*) as count FROM user_data_schema.stm_good WHERE user_id = $1', [user.id]),
        client.query('SELECT COUNT(*) as count FROM user_data_schema.stm_bad WHERE user_id = $1', [user.id]),
        client.query('SELECT COUNT(*) as count FROM user_data_schema.mcl_chains WHERE user_id = $1', [user.id]),
        client.query('SELECT COUNT(*) as count FROM user_data_schema.memory_embeddings WHERE user_id = $1', [user.id]),
      ]);

      const goodCount = parseInt(goodResult.rows[0].count);
      const badCount = parseInt(badResult.rows[0].count);
      const mclCount = parseInt(mclResult.rows[0].count);
      const memoryCount = parseInt(memoryResult.rows[0].count);
...
async function getPostgresConnectionString(projectId: string): Promise<string | null> {
  try {
    ...
    const postgresAddon = addonsData.data?.addons?.find((a: any) => a.spec?.type === 'postgresql');
